...
- Each **operational site** hosts one InDevelopment asset at a time and frees the instant that asset leaves development, including between phases. Agents start with 3 sites. The cap gates only *new* trial starts; ongoing trials are never blocked
...
- **Upgrade action** (`0`/`1`): buy one new site. Cost follows a Fibonacci curve off a £500M base (1×, 1×, 2×, 3×, 5×, … for successive purchases beyond the starting 3), rounded to £1M. A bought site takes 2 steps to build before it becomes operational
