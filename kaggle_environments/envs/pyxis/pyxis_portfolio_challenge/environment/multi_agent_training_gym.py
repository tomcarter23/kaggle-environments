...
class MultiAgentInvestmentGameEnv(ParallelEnv):
    ...
    def __init__(
        ...
        self._indication_features = (
            6 if (marketing_config is not None and marketing_config.enabled) else 5
        )

        if (
            self.investment_levels_config is not None
            and self.investment_levels_config.enabled
            and self.drop_action_config is not None
        ...
        bd_ptrs_readings_extras = (
            ...
            if ptrs_readings_config is not None and ptrs_readings_config.enabled
            else 0
        )
    ...
    def _sites_on(self) -> bool:
        ...
        return (
            self.clinical_sites_config is not None
            and self.clinical_sites_config.enabled
        )
    ...
    def action_space(self, agent: str) -> gym.Space:
        ...
        use_drop = (
            self.drop_action_config is not None and self.drop_action_config.enabled
        )
        if use_levels:
        ...
        if self.ptrs_readings_config is not None:
        ...
        if self.marketing_config is not None and self.marketing_config.enabled:
    ...
    def _parse_actions(self, actions: dict[str, Any]) -> dict[str, dict]:
        ...
        for agent in self.agents:
            ...
            # Site-action defaults (used unless overridden in the dict branch).
            upgrade[agent] = 0
            site_bid[agent] = 0.0
            site_priority[agent] = np.zeros(self.max_num_assets, dtype=np.float32)
            if action is None:
                ...
                ptrs_research[agent] = np.zeros(num_ptrs_research_slots, dtype=np.int64)
                ...
                bd_bids[agent] = self._decode_bd_bids(raw_bids)
                if self.pricing_config.enabled:
                ...
                raw_dc = action.get("demand_creation", np.zeros(num_ind_slots))
                demand_creation[agent] = np.asarray(raw_dc, dtype=np.int64)
                raw_be = action.get("brand_equity", np.zeros(self.max_num_assets))
                brand_equity[agent] = np.asarray(raw_be, dtype=np.int64)
                raw_ptr = action.get("ptrs_research", np.zeros(num_ptrs_research_slots))
                ptrs_research[agent] = np.asarray(raw_ptr, dtype=np.int64)
                if self._sites_on:
                    upgrade[agent] = int(np.rint(action.get("upgrade", 0)))
                if self._site_auction_on:
                    site_bid[agent] = self._decode_site_bid(
                        action.get("site_bid", 0.0)
                    )
                if self._site_priority_on:
                    site_priority[agent] = np.asarray(
                        action.get(
                            "site_priority",
                            np.zeros(self.max_num_assets),
                        ),
                        dtype=np.float32,
                    )
            else:
    ...
    def flatten_dict_obs(self, dict_obs: dict) -> np.ndarray:
        ...
        ptrs_readings_on_flat = (
            self.ptrs_readings_config is not None and self.ptrs_readings_config.enabled
        )
