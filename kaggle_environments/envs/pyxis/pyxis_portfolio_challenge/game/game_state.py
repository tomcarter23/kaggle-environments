...
class GameState(BaseModel):
    ...
    def initialise_new_game(
        ...
        **asset_generator_kwargs,
        ...
        seed : int, optional
            Random seed. If provided, re-initializes the game-wide RNG with this seed.
        ...
        **asset_generator_kwargs : dict
        ...
        drop_action_config = asset_generator_kwargs.pop("drop_action_config", None)
        marketing_config = asset_generator_kwargs.pop("marketing_config", None)
        clinical_sites_config = asset_generator_kwargs.pop(
            "clinical_sites_config", None
        )
        # ptrs_readings_config is needed by both GameState and asset generator
        ptrs_readings_config = asset_generator_kwargs.get("ptrs_readings_config", None)
        ...
        asset_generator = asset_generator_cls(**asset_generator_kwargs)
        ...
        initial_operational_sites = (
            ...
            if clinical_sites_config is not None and clinical_sites_config.enabled
            else 0
        )
    ...
    def step(
        ...
        if marketing_cfg is not None and marketing_cfg.enabled:
            ...
            if demand_creation_actions is not None:
                cost = marketing_cfg.dc_cost_fraction * demand_creation_cost_base
            ...
            if brand_equity_actions is not None:
                for asset_id, asset in assets_for_step.items():
                    if brand_equity_actions.get(asset_id, 0) == 1:
                        cost = marketing_cfg.be_cost_fraction * asset.max_revenue
                        current_cash -= cost
                        current_realised_cost += cost
